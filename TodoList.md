...
- [x] Verify public endpoints have a production-grade distributed rate limit; the current in-memory limiter resets on deployment and is not shared between instances.
  - Evidence: `npm run test:integration:events` passed on branch `feature/distributed-public-rate-limits`; includes `Public event endpoints enforce distributed rate limits`.
